...
# Adjusted target variables (Churn1 to Churn90)
target_variables = [f'Churn{i}' for i in range(8, 91)]
...
with Tee('ViviraChurn.txt'):
    # Iterate over feature sets
    for i in range(1, 84):  # Adjusted range according to the logic provided
        exercise_features = [f'exercise{j}' for j in range(1, i + 1)]
        active_features = [f'active{j}' for j in range(1, i + 1)]
        ...
        target = f'Churn{i + 7}'  # Corresponding churn target
        ...
        # use class_weight='balanced' fo better prediction on Days 1-4
        ...
        churned_users = df[f'Churn{i}'] == 1
        churning_users = (df[f'Churn{i}'] == 0) & (df[target] == 1)
        active_users = df[target] == 0
        ...
        # Store results
        metrics_storage['accuracy'].loc[f'Day{i}', target] = accuracy
        metrics_storage['f1'].loc[f'Day{i}', target] = f1
        metrics_storage['auc'].loc[f'Day{i}', target] = auc
        metrics_storage['precision'].loc[f'Day{i}', target] = precision
        metrics_storage['recall'].loc[f'Day{i}', target] = recall

        # Print results
        print(f"Results for {target} with feature set 1-{i}:")
