...
# Load dataset
# manoa_filtered.csv only contains users who completed the initial blood pressure measurement week 
df = pd.read_csv('manoa_filtered.csv')

# Adjusted target variables (NonAdhMonth2 to NonAdhMonth6)
target_variables = [f'NonAdhMonth{i}' for i in range(2, 7)]
...
with Tee('ManoaNonadherence.txt'):
    # Iterate over target variables and corresponding feature sets
    for month in range(2, 7):
        # Define the correct range of features for the current month
        logins_features = [f'logins{j}' for j in range(1, (month-1)*31 + 1)]
        active_features = [f'active{j}' for j in range(1, (month-1)*31 + 1)]

        X = df[logins_features + active_features]
        ...
        # Categorization based on LastMonth status
        churned_users = df['LastMonth'] < month
        churning_users = df['LastMonth'] == month
        active_users = df['LastMonth'] > month
        ...
        # Store results
        metrics_storage['accuracy'].loc[f'Month{month}', target] = accuracy
        metrics_storage['f1'].loc[f'Month{month}', target] = f1
        metrics_storage['auc'].loc[f'Month{month}', target] = auc
        metrics_storage['precision'].loc[f'Month{month}', target] = precision
        metrics_storage['recall'].loc[f'Month{month}', target] = recall

        # Print results
        print(f"Results for {target} with feature set up to month {month-1}:")
        ...
        feature_names = logins_features + active_features
        print("Feature Importances:")
